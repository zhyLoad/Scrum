    ...
    if request.method == 'POST':
        try:
            ...
            if flag!=0:
                ...
                return HttpResponse(json.dumps(ret), content_type="application/json")
            try:
                data = Users.objects.create(username=username,password=hashlib.sha512(password.encode("utf8")).hexdigest(),name=name,email=email,created=datetime.datetime.now().isoformat())
